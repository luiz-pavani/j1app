...
const DB_HANSOKU_REASONS = ["Head Dive (Mergulho)", "Ponte (Head Defense)", "Kani Basami", "Kawazu Gake", "Ato contra o espírito do Judô", "Outros"];
...
export default function JudoPlayer() {
  ...
  const [motivoHansoku, setMotivoHansoku] = useState(DB_HANSOKU_REASONS[0]);
  const [matchDuration, setMatchDuration] = useState(240); // 4 minutos em segundos
  ...
  const [punicaoMode, setPunicaoMode] = useState<string | null>(null);
   ...
   // States para novos recursos
   const [modalSoremade, setModalSoremade] = useState(false);
   const [showSoremadeAnimation, setShowSoremadeAnimation] = useState(false);
   const [matchResult, setMatchResult] = useState<'IPPON' | 'HANSOKU' | 'DRAW' | null>(null);
   const [resultWinner, setResultWinner] = useState<'BRANCO' | 'AZUL' | 'EMPATE' | null>(null);
   const [modalResult, setModalResult] = useState(false);
   const [matchStats, setMatchStats] = useState({
      totalEvents: 0, totalIppons: 0, totalWazas: 0, totalYukos: 0,
      totalShidoBranco: 0, totalShidoAzul: 0, totalAtaquesBranco: 0, totalAtaquesAzul: 0
   });
  ...
  // --- ATUALIZAR CURRENTTIME DURANTE A REPRODUÇÃO ---
  useEffect(() => {
  ...
  const tempoDisplay = useMemo(() => {
    ...
    const isGoldenScore = elapsed > matchDuration;
    if (!isGoldenScore) {
      // TEMPO REGULAR: Countdown (regressivo)
      const remaining = Math.max(0, matchDuration - elapsed);
      return { text: "TEMPO REGULAR", time: `${Math.floor(remaining/60)}:${Math.floor(remaining%60).toString().padStart(2,'0')}`, isGS: false };
    } else {
      // GOLDEN SCORE: Count up (progressivo)
      const gsElapsed = elapsed - matchDuration;
      return { text: "GOLDEN SCORE", time: `${Math.floor(gsElapsed/60)}:${Math.floor(gsElapsed%60).toString().padStart(2,'0')}`, isGS: true };
    }
  }, [accumulatedFightTime, matchDuration]);
  ...
  const placar = useMemo(() => {
    const p = { branco: { ippon:0, waza:0, yuko:0, shido:0, hansoku:0 }, azul: { ippon:0, waza:0, yuko:0, shido:0, hansoku:0 } };
    eventos.filter((ev:any) => ev.videoId === currentVideo.name).forEach((ev: any) => {
      ...
      if (ev.resultado && (ev.resultado.includes('yuko') || ev.resultado === 'Yuko')) q.yuko++;
      if (ev.categoria === 'PUNICAO') { 
        if (ev.tipo === 'SHIDO') q.shido++; 
        if (ev.tipo === 'HANSOKU') {
          q.hansoku++;
          q.shido = 3;
        }
    ...
    if (p.branco.shido >= 3) p.branco.hansoku = 1;
    if (p.azul.shido >= 3) p.azul.hansoku = 1;
      // Regra: 2 Waza-ari = IPPON (converte e zera Waza)
      if (p.branco.waza >= 2) { p.branco.ippon++; p.branco.waza = 0; }
      if (p.azul.waza >= 2) { p.azul.ippon++; p.azul.waza = 0; }
    return p;
  }, [eventos, currentVideo]);
  ...
  const registrarFluxo = async (tipo: string) => { const id = Date.now(); const novo = { id, videoId: currentVideo.name, tempo: currentTime, categoria: 'FLUXO', tipo, atleta: '-', lado: '-', corTecnica: THEME.neutral }; setEventos(prev => [novo, ...prev]); await supabase.from('events').insert([{ id, video_id: currentVideo.name, tempo: currentTime, categoria: 'FLUXO', tipo, atleta: '-', lado: '-', cor_tecnica: THEME.neutral }]); };
   ...
   // --- UTILITÁRIAS: Feedback, Stats e Replay (declaradas antes do uso) ---
   const playSoremadeSound = () => {
      try {
   ...
   const triggerSoremadeFeedback = () => {
      playSoremadeSound();
      setShowSoremadeAnimation(true);
      setModalSoremade(true);
      setTimeout(() => setShowSoremadeAnimation(false), 2000);
   };

   const calculateMatchStats = () => {
      const videoEventos = eventos.filter((e: any) => e.videoId === currentVideo.name);
      const stats = {
         totalEvents: videoEventos.length,
         totalIppons: videoEventos.filter((e: any) => e.tipo?.includes('IPPON')).length,
         totalWazas: videoEventos.filter((e: any) => e.tipo === 'WAZA_ARIARI_DIRETO' || e.resultado === 'Waza-ari').length,
         totalYukos: videoEventos.filter((e: any) => e.tipo === 'YUKO_DIRETO' || e.resultado === 'Yuko').length,
         totalShidoBranco: videoEventos.filter((e: any) => e.tipo === 'SHIDO' && e.atleta === 'BRANCO').length,
         totalShidoAzul: videoEventos.filter((e: any) => e.tipo === 'SHIDO' && e.atleta === 'AZUL').length,
         totalAtaquesBranco: videoEventos.filter((e: any) => (e.categoria === 'TECNICA' || e.categoria === 'NE-WAZA') && e.atleta === 'BRANCO').length,
         totalAtaquesAzul: videoEventos.filter((e: any) => (e.categoria === 'TECNICA' || e.categoria === 'NE-WAZA') && e.atleta === 'AZUL').length,
      };
      setMatchStats(stats);
      return stats;
   };

   const saveMatchResult = async (result: 'IPPON' | 'HANSOKU' | 'DRAW', winner: 'BRANCO' | 'AZUL' | 'EMPATE') => {
      const resultData = {
         id: Date.now(),
         video_id: currentVideo.id,
         resultado: result,
         vencedor: winner,
         tempo_match: accumulatedFightTime,
         placar_final: placar,
         stats: matchStats,
   ...
   const replayLastCriticalMoment = (eventId?: number) => {
      const ev = eventId ? eventos.find((e:any) => e.id === eventId) : eventos.find((e:any) => e.videoId === currentVideo.name && (e.tipo === 'IPPON_DIRETO' || e.tipo === 'HANSOKU' || e.tipo === 'YUKO_DIRETO' || e.tipo === 'WAZA_ARIARI_DIRETO' || e.tipo === 'SOREMADE'));
      if (!ev) return alert('Nenhum momento crítico encontrado para replay.');
      const start = Math.max(0, (ev.tempo || 0) - 10);
      if (currentVideo.type === 'YOUTUBE' && youtubePlayerRef.current) {
         youtubePlayerRef.current.seekTo(start, true);
         youtubePlayerRef.current.playVideo();
      } else if (filePlayerRef.current) {
         filePlayerRef.current.currentTime = start;
         filePlayerRef.current.play();
      }
      setIsPlaying(true);
   };
  
   const checkAutoSoremade = async () => {
      // Verifica se deve registrar SOREMADE automaticamente (Ippon, Hansoku, 3 Shidos, GS rules)
      if (eventos.length === 0) return;
      const lastEvent = eventos[0];
      const lastWasSoremade = lastEvent.tipo === 'SOREMADE';
  ...
  const registrarYukoDireto = async (atleta: string) => { const id = Date.now(); const novo = { id, videoId: currentVideo.name, tempo: currentTime, categoria: 'TECNICA', tipo: 'YUKO_DIRETO', resultado: 'Yuko', especifico: 'Yuko Direto', atleta, lado: '-', corTecnica: THEME.kumi }; setEventos(prev => [novo, ...prev]); await supabase.from('events').insert([{ id, video_id: currentVideo.name, tempo: currentTime, categoria: 'TECNICA', tipo: 'YUKO_DIRETO', resultado: 'Yuko', especifico: 'Yuko Direto', atleta, lado: '-', cor_tecnica: THEME.kumi }]); };
  
  const registrarHansokuDireto = async (atleta: string) => { const id = Date.now(); const novo = { id, videoId: currentVideo.name, tempo: currentTime, categoria: 'PUNICAO', tipo: 'HANSOKU', especifico: motivoHansoku, atleta, lado: '-', corTecnica: THEME.danger }; setEventos(prev => [novo, ...prev]); await supabase.from('events').insert([{ id, video_id: currentVideo.name, tempo: currentTime, categoria: 'PUNICAO', tipo: 'HANSOKU', especifico: motivoHansoku, atleta, lado: '-', cor_tecnica: THEME.danger }]); registrarFluxo('SOREMADE'); };
  ...
  // DESACOPLAR VÍDEO DO CRONÔMETRO: Apenas registra estado, sem tocar no vídeo
  ...
  outline: 'none'
      ...
      <div className="no-print" style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
        <h1 style={{ margin: 0, fontSize: isMobile?'24px':'32px', fontWeight: '800', letterSpacing: '-1px', display: 'flex', alignItems: 'center' }}>
          ...
          <div><span style={{ color: 'white' }}>SMAART</span><span style={{ color: THEME.primary }}>PRO J1</span><div style={{fontSize:'10px', color: THEME.textDim, fontWeight:'400', letterSpacing:'2px', marginTop:'-4px'}}>ELITE JUDO ANALYTICS</div></div>
<span style={{ fontSize: '10px', color: THEME.text, marginLeft: '12px', background: THEME.cardBorder, padding: '4px 8px', borderRadius: '20px', border:`1px solid rgba(255,255,255,0.1)` }}>BETA 3</span>
        </h1>
      ...
      {mode === 'META' ? (
         ...
         // Adicionamos flex: 1, minHeight: 0 e overflow: hidden
<div className="no-print" style={{ display: 'flex', gap: '24px', alignItems: 'stretch', flexDirection: isMobile ? 'column' : 'row', flex: 1, minHeight: 0, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 3, width: '100%', display:'flex', flexDirection:'column', gap:'20px' }}>
              ...
              <div ref={playerContainerRef} style={{ ...cardStyle, position: isDataFullscreen ? 'fixed' : 'relative', top: isDataFullscreen ? 0 : 'auto', left: isDataFullscreen ? 0 : 'auto', width: isDataFullscreen ? '100vw' : '100%', height: isDataFullscreen ? '100vh' : 'auto', paddingTop: isDataFullscreen ? 0 : '56.25%', zIndex: isDataFullscreen ? 999 : 1, boxShadow: '0 20px 40px -10px rgba(0,0,0,0.5)' }}>
                   ...
                   {/* LÓGICA DE TELA INICIAL vs PLAYER */}
{playlist.length === 0 ? (
  <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: THEME.bg, color: THEME.textDim, gap: '20px', zIndex: 50 }}>
      <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: THEME.card, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 30px ${THEME.primary}44` }}>
          <Video size={40} color={THEME.primary} />
      </div>
      <h2 style={{ fontSize: '24px', fontWeight: '800', color: 'white', margin: 0 }}>Comece sua Análise</h2>
      <p style={{ maxWidth: '400px', textAlign: 'center', lineHeight: '1.5' }}>Carregue um vídeo do seu computador ou cole um link do YouTube para iniciar o rastreamento.</p>
      
      <div style={{ display: 'flex', gap: '15px' }}>
          <button onClick={() => fileInputRef.current.click()} style={{ ...btnStyle, background: THEME.primary, color: 'white', padding: '12px 24px', fontSize: '14px', boxShadow: '0 4px 15px rgba(59, 130, 246, 0.4)' }}>
  ...
  <>
    {/* AQUI DENTRO MANTENHA O CÓDIGO ANTIGO DO PLAYER (YouTube/Video File) */}
    {currentVideo.type === 'YOUTUBE' ? (
       ...
       <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background:'black' }}><video ref={filePlayerRef} src={currentVideo.id} style={{width:'100%', height:'100%', objectFit:'contain'}} controls={false} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={onFileEnded} onLoadedMetadata={(e:any) => setDuration(e.target.duration)}/></div>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                <button onClick={() => { registrarFluxo('SOREMADE'); triggerSoremadeFeedback(); }} style={{...btnStyle, flex:1, background: THEME.card, border:`1px solid ${THEME.cardBorder}`, color:THEME.text, padding:'16px', borderRadius:'12px', fontSize:'13px', fontWeight:'700'}}><Flag size={18}/> SOREMADE</button>
                <button onClick={() => { setShowStatsModal(true); calculateMatchStats(); }} style={{...btnStyle, flex:1, background:THEME.bg, border:`1px solid ${THEME.cardBorder}`, color:THEME.text, padding:'16px', borderRadius:'12px', fontSize:'13px', fontWeight:'700'}}><BarChart2 size={16}/> STATS</button>
                            <button onClick={() => setShowStatsModal(true)} style={{...btnStyle, flex:1, background:THEME.primary, color:'white', padding:'16px', borderRadius:'12px', fontSize:'13px', fontWeight:'700'}} title="Ver Estatísticas da Luta"><BarChart2 size={18}/> Stats</button>
              </div>
              ...
              <div style={{...cardStyle, padding:'16px', display:'flex', alignItems:'center', gap:'12px'}}>
                 ...
                 <select style={{flex:1, background: THEME.bg, color: THEME.text, border:`1px solid ${THEME.cardBorder}`, padding:'12px', borderRadius:'8px', fontSize: '13px', outline:'none'}} onChange={(e) => setMotivoHansoku(e.target.value)} value={motivoHansoku}>{DB_HANSOKU_REASONS.map(m => <option key={m} value={m}>{m}</option>)}</select>
                 <button onClick={() => registrarHansokuDireto('BRANCO')} style={{...btnStyle, width:'48px', height:'44px', background:'#e2e8f0', color:'#0f172a', fontSize:'14px', borderRadius:'8px', fontWeight:'700'}}>H⚪</button>
                 <button onClick={() => registrarHansokuDireto('AZUL')} style={{...btnStyle, width:'48px', height:'44px', background: THEME.primary, color:'white', fontSize:'14px', borderRadius:'8px', fontWeight:'700'}}>H🔵</button>
              </div>
            </div>

            {/* RIGHT COLUMN (DASHBOARD + LOG) */}
            <div className="no-print" style={{ flex: 2, width: '100%', display:'flex', flexDirection:'column', gap:'20px', overflow: 'hidden' }}>
              ...
              <div style={{ ...cardStyle, padding: '0', display: 'grid', gridTemplateColumns: '1.2fr 1fr 1.2fr', overflow:'hidden' }}>
                <div style={{ background:'white', padding:'20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent:'center', borderRight:`4px solid ${THEME.bg}` }}>
                     ...
                     <div style={{display: 'flex', gap: '10px', marginTop: '12px'}}>
                        ...
                        <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: '#64748b', fontWeight:'700'}}>W</div><div style={{fontSize:'28px', fontWeight:'800', color:'#0f172a', lineHeight:'1'}}>{placar.branco.waza}</div></div>
                        <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: '#64748b', fontWeight:'700'}}>Y</div><div style={{fontSize:'28px', fontWeight:'800', color:'#0f172a', lineHeight:'1'}}>{placar.branco.yuko}</div></div>
                        <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: '#eab308', fontWeight:'700'}}>S</div><div style={{fontSize:'28px', fontWeight:'800', color:'#eab308', lineHeight:'1'}}>{placar.branco.shido}</div></div>
                        {placar.branco.hansoku > 0 && <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: '#ef4444', fontWeight:'700'}}>H</div><div style={{fontSize:'28px', fontWeight:'800', color:'#ef4444', lineHeight:'1'}}>✕</div></div>}
                     </div>
                </div>
                {/* TIMER */}
                <div style={{ background: THEME.card, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding:'12px' }}>
                     ...
                     {!isFightActive && <div style={{marginTop:'8px', display:'flex', alignItems:'center', gap:'6px', fontSize:'11px'}}>
                        <span style={{color: THEME.textDim}}>Duração (s):</span>
                        <input type="number" min="60" max="600" step="30" value={matchDuration} onChange={(e) => setMatchDuration(Math.max(60, parseInt(e.target.value) || 240))} style={{width:'50px', padding:'4px', borderRadius:'4px', border:`1px solid ${THEME.cardBorder}`, background: THEME.bg, color: THEME.text, fontSize:'11px', textAlign:'center'}}/>
                     </div>}
                </div>
                {/* BLUE */}
                <div style={{ background: THEME.primary, padding:'20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent:'center', borderLeft:`4px solid ${THEME.bg}` }}>
                     ...
                     <div style={{display: 'flex', gap: '10px', marginTop: '12px'}}>
                        ...
                        <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: 'rgba(255,255,255,0.6)', fontWeight:'700'}}>W</div><div style={{fontSize:'28px', fontWeight:'800', color:'white', lineHeight:'1'}}>{placar.azul.waza}</div></div>
                        <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: 'rgba(255,255,255,0.6)', fontWeight:'700'}}>Y</div><div style={{fontSize:'28px', fontWeight:'800', color:'white', lineHeight:'1'}}>{placar.azul.yuko}</div></div>
                        <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: '#eab308', fontWeight:'700'}}>S</div><div style={{fontSize:'28px', fontWeight:'800', color:'#eab308', lineHeight:'1'}}>{placar.azul.shido}</div></div>
                        {placar.azul.hansoku > 0 && <div style={{textAlign:'center'}}><div style={{fontSize:'9px', color: '#ef4444', fontWeight:'700'}}>H</div><div style={{fontSize:'28px', fontWeight:'800', color:'#ef4444', lineHeight:'1'}}>✕</div></div>}
                     </div>
                </div>
              </div>
              ...
              {/* --- EVENT LOG & FILTERS --- */}
              <div style={{...cardStyle, flex: 1, display:'flex', flexDirection:'column', overflow:'hidden', minHeight: 0 }}>
                  {/* Filter Tabs */}
                          ...
                          filteredEventos.sort((a:any, b:any) => b.tempo - a.tempo).map((ev: any) => (
                              <div key={ev.id} onClick={() => editarEvento(ev)} className="glow-hover" style={{padding:'12px 16px', borderBottom:`1px solid ${THEME.cardBorder}`, display:'flex', gap:'12px', alignItems:'center', cursor:'pointer', borderLeft: `4px solid ${ev.corTecnica || THEME.neutral}`, background: ev.id === editingEventId ? `${THEME.primary}22` : 'transparent'}}>
                                  ...
                                  <div style={{display:'flex', flexDirection:'column', gap:'6px', alignItems:'flex-end'}}>
                                     {/* BOTÕES: DELETAR, PLAY, REPLAY, COPY */}
                                     <div style={{display:'flex', gap:'6px', flexDirection:'row'}}>
                                        <button onClick={(e) => { e.stopPropagation(); deletarEvento(ev.id, e); }} style={{background:'transparent', border:'none', color:THEME.danger, cursor:'pointer', padding:'6px', opacity: 0.9}} title="Apagar Evento"><Trash2 size={16}/></button>
                                        <button onClick={(e) => { e.stopPropagation(); if(currentVideo.type==='YOUTUBE') youtubePlayerRef.current.seekTo(ev.tempo, true); else filePlayerRef.current.currentTime = ev.tempo; }} style={{background:'transparent', border:'none', color:THEME.primary, cursor:'pointer', padding:'6px'}} title="Ir para tempo"><PlayCircle size={16}/></button>
                                        <button onClick={(e) => { e.stopPropagation(); replayLastCriticalMoment(ev.id); }} style={{background:'transparent', border:'none', color:'white', cursor:'pointer', padding:'6px'}} title="Replay 10s antes"><RotateCw size={16}/></button>
                                        <button onClick={(e) => { e.stopPropagation(); try { navigator.clipboard.writeText(JSON.stringify(ev)); } catch(err){ console.warn('Clipboard fail', err); } }} style={{background:'transparent', border:'none', color:THEME.textDim, cursor:'pointer', padding:'6px'}} title="Copiar JSON"><Copy size={16}/></button>
                                     </div>
                                  </div>
                              </div>
                          ))
      ...
      {/* MODAL SOREMADE - Animação + Feedback Visual */}
      {modalSoremade && (
         <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.95)', zIndex:4000, display:'flex', alignItems:'center', justifyContent:'center'}} onClick={() => setModalSoremade(false)}>
            <div style={{textAlign:'center', opacity: showSoremadeAnimation ? 1 : 0.7, transform: showSoremadeAnimation ? 'scale(1.2)' : 'scale(1)', transition:'all 0.3s ease'}}>
               <div style={{fontSize:'120px', fontWeight:'900', color:THEME.danger, marginBottom:'20px', textShadow:'0 0 30px rgba(239,68,68,0.8)', animation: showSoremadeAnimation ? 'pulse 0.5s ease-in-out infinite' : 'none'}}>
                  SOREMADE
               </div>
               <div style={{fontSize:'24px', color:'white', marginBottom:'40px', fontWeight:'700', letterSpacing:'2px'}}>LUTA ENCERRADA</div>
               <button onClick={() => { setModalSoremade(false); setModalResult(true); }} style={{padding:'16px 32px', fontSize:'16px', fontWeight:'700', background:THEME.primary, color:'white', border:'none', borderRadius:'12px', cursor:'pointer', marginTop:'30px'}}>
                  Registrar Resultado
               </button>
            </div>
         </div>
      ...
      {/* MODAL RESULT RECORDING */}
      {modalResult && (
         <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.8)', zIndex:3900, display:'flex', alignItems:'center', justifyContent:'center'}}>
            <div style={{background:THEME.bg, width:'90%', maxWidth:'600px', padding:'40px', borderRadius:'20px', border:`2px solid ${THEME.primary}`}}>
               <h2 style={{margin:'0 0 30px 0', color:THEME.primary, textAlign:'center'}}>Registrar Resultado Final</h2>
               
               <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'30px'}}>
                  <button onClick={() => { saveMatchResult('IPPON', 'BRANCO'); }} style={{padding:'20px', background:THEME.success, color:'white', border:'none', borderRadius:'12px', fontWeight:'700', cursor:'pointer', fontSize:'16px'}}>
                     ✕ BRANCO (Ippon)
                  </button>
                  <button onClick={() => { saveMatchResult('IPPON', 'AZUL'); }} style={{padding:'20px', background:THEME.primary, color:'white', border:'none', borderRadius:'12px', fontWeight:'700', cursor:'pointer', fontSize:'16px'}}>
                     ✕ AZUL (Ippon)
                  </button>
                  <button onClick={() => { saveMatchResult('HANSOKU', 'BRANCO'); }} style={{padding:'20px', background:THEME.danger, color:'white', border:'none', borderRadius:'12px', fontWeight:'700', cursor:'pointer', fontSize:'16px'}}>
                     🚫 BRANCO (Hansoku)
                  </button>
                  <button onClick={() => { saveMatchResult('HANSOKU', 'AZUL'); }} style={{padding:'20px', background:THEME.danger, color:'white', border:'none', borderRadius:'12px', fontWeight:'700', cursor:'pointer', fontSize:'16px'}}>
                     🚫 AZUL (Hansoku)
                  </button>
                  <button onClick={() => { saveMatchResult('DRAW', 'EMPATE'); }} style={{padding:'20px', background:'#8b5cf6', color:'white', border:'none', borderRadius:'12px', fontWeight:'700', cursor:'pointer', fontSize:'16px', gridColumn:'1/-1'}}>
                     ⚖️ EMPATE
                  </button>
               </div>

               <div style={{display:'flex', gap:'10px'}}>
                  <button onClick={() => { setModalResult(false); setShowStatsModal(true); }} style={{flex:1, padding:'12px', background:THEME.warning, border:'none', borderRadius:'8px', fontWeight:'700', cursor:'pointer'}}>
                     Ver Estatísticas
                  </button>
                  <button onClick={() => setModalResult(false)} style={{flex:1, padding:'12px', background:'transparent', border:`1px solid ${THEME.cardBorder}`, color:THEME.textDim, borderRadius:'8px', cursor:'pointer'}}>
                     Fechar
                  </button>
               </div>
            </div>
      ...
      {/* MODAL MATCH STATISTICS DASHBOARD */}
      {showStatsModal && (
         <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.9)', zIndex:3850, display:'flex', alignItems:'center', justifyContent:'center'}}>
            <div style={{background:THEME.bg, width:'95%', maxWidth:'800px', maxHeight:'90vh', padding:'40px', borderRadius:'20px', border:`2px solid ${THEME.primary}`, overflowY:'auto'}}>
               <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'30px'}}>
                  <h2 style={{margin:0, color:THEME.primary}}>📊 Estatísticas da Luta</h2>
                  <button onClick={() => setShowStatsModal(false)} style={{background:'none', border:'none', color:'white', cursor:'pointer', fontSize:'24px'}}>✕</button>
               </div>

               <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'30px'}}>
                  {/* Stats BRANCO */}
                  <div style={{background:THEME.card, padding:'20px', borderRadius:'12px', border:`1px solid ${THEME.cardBorder}`}}>
                     <div style={{fontSize:'18px', fontWeight:'800', color:'white', marginBottom:'15px'}}>⚪ BRANCO</div>
                     <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                        <span style={{color:THEME.textDim}}>Ataques:</span>
                        <span style={{color:'white', fontWeight:'700'}}>{matchStats.totalAtaquesBranco}</span>
                     </div>
                     <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                        <span style={{color:THEME.textDim}}>Shidos:</span>
                        <span style={{color:'#eab308', fontWeight:'700'}}>{matchStats.totalShidoBranco}</span>
                     </div>
                     <div style={{display:'flex', justifyContent:'space-between'}}>
                        <span style={{color:THEME.textDim}}>Placar Final:</span>
                        <span style={{color:THEME.success, fontWeight:'700'}}>I:{placar.branco.ippon} W:{placar.branco.waza} S:{placar.branco.shido}</span>
                     </div>
                  </div>

                  {/* Stats AZUL */}
                  <div style={{background:THEME.card, padding:'20px', borderRadius:'12px', border:`1px solid ${THEME.cardBorder}`}}>
                     <div style={{fontSize:'18px', fontWeight:'800', color:THEME.primary, marginBottom:'15px'}}>🔵 AZUL</div>
                     <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                        <span style={{color:THEME.textDim}}>Ataques:</span>
                        <span style={{color:'white', fontWeight:'700'}}>{matchStats.totalAtaquesAzul}</span>
                     </div>
                     <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                        <span style={{color:THEME.textDim}}>Shidos:</span>
                        <span style={{color:'#facc15', fontWeight:'700'}}>{matchStats.totalShidoAzul}</span>
                     </div>
                     <div style={{display:'flex', justifyContent:'space-between'}}>
                        <span style={{color:THEME.textDim}}>Placar Final:</span>
                        <span style={{color:THEME.primary, fontWeight:'700'}}>I:{placar.azul.ippon} W:{placar.azul.waza} S:{placar.azul.shido}</span>
                     </div>
                  </div>
               </div>

               {/* Resumo Geral */}
               <div style={{background:THEME.card, padding:'20px', borderRadius:'12px', border:`1px solid ${THEME.cardBorder}`, marginBottom:'30px'}}>
                  <div style={{fontSize:'16px', fontWeight:'800', color:THEME.warning, marginBottom:'15px'}}>⏱️ Resumo Geral</div>
                  <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:'10px', fontSize:'14px'}}>
                     <div><span style={{color:THEME.textDim}}>Total Eventos:</span><br/><span style={{color:'white', fontWeight:'700', fontSize:'18px'}}>{matchStats.totalEvents}</span></div>
                     <div><span style={{color:THEME.textDim}}>Ippons:</span><br/><span style={{color:THEME.success, fontWeight:'700', fontSize:'18px'}}>{matchStats.totalIppons}</span></div>
                     <div><span style={{color:THEME.textDim}}>Waza-aris:</span><br/><span style={{color:'#10b981', fontWeight:'700', fontSize:'18px'}}>{matchStats.totalWazas}</span></div>
                     <div><span style={{color:THEME.textDim}}>Tempo Total:</span><br/><span style={{color:THEME.warning, fontWeight:'700', fontSize:'18px'}}>{Math.floor(accumulatedFightTime/60)}:{String(Math.floor(accumulatedFightTime%60)).padStart(2,'0')}</span></div>
                  </div>
               </div>

               <div style={{display:'flex', gap:'10px'}}>
                  <button onClick={() => setShowStatsModal(false)} style={{flex:1, padding:'12px', background:THEME.primary, border:'none', borderRadius:'8px', color:'white', fontWeight:'700', cursor:'pointer'}}>
                     Fechar
                  </button>
               </div>
            </div>
